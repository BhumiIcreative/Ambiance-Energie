...
class AccountMoveImport(models.TransientModel):
    ...
    def _partner_speed_dict(self):
        ...
        partner_sr = self.env['res.partner'].with_context(company_id=company_id).search_read(
            [
                '|',
    ...
    def create_moves_from_pivot(self, pivot, post=False):
        ...
        acc_speed_dict = {}
        acc_sr = self.env['account.account'].with_context(company_id=company_id).search_read([
            ('company_id', '=', company_id),
            ('deprecated', '=', False)], ['code'])
        for l in acc_sr:
            acc_speed_dict[l['code'].upper()] = l['id']
        aacc_speed_dict = {}
        aacc_sr = self.env['account.analytic.account'].with_context(company_id=company_id).search_read(
            [('company_id', '=', company_id), ('code', '!=', False)],
            ['code'])
        for l in aacc_sr:
            aacc_speed_dict[l['code'].upper()] = l['id']
        journal_speed_dict = {}
        journal_sr = self.env['account.journal'].with_context(company_id=company_id).search_read([
        ...
        ## ROMARIC
        logger.warning("company_id %s " % company_id)  
