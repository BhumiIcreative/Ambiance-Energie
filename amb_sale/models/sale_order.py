...
class SaleOrder(models.Model):
    ...
    def granule_product_proposal(self):
        ...
        # Check if the order type is 'gran' (granule) and if the granule product is not already in the order
        if self.type_order == 'gran' and article_granule not in self.order_line.mapped('product_id').ids:
            ...
            if sub:
                ...
                # Calculate the number of granules based on the subscription amount and the price of the granule
                nb_granule = floor(sub.amount / (price_article + vat.amount / 100 * price_article))
                fiscal_position = self.fiscal_position_id

                # Get the product account for the granule product based on fiscal position
