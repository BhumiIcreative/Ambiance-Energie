...
class PaymentTimeLine(models.Model):
    ...
    _description = _('Payment timeline')
    _order = 'date'

    @api.model
    def _get_default_currency(self):
        return self.env['account.move']._get_default_currency()

    amount = Monetary(_('Amount'), required=True)
    date = Date(_('Date'), required=True, default=Date.today())

    currency_id = Many2one('res.currency', store=True, readonly=True,
        required=True, string=_('Currency'), default=_get_default_currency)
    move_id = Many2one('account.move', string=_('Move'))
    sale_id = Many2one('sale.order', string=_('Sale'))
    payment_instrument_id = Many2one('sale_timeline.payment.instrument', string=_('Payment method'))
    advance = Boolean('Acompte', default=False, readonly=True)

    
# Suppression de la condition sur la date ?
#    @api.onchange('date')
#    def _onchange_date(self):
#        if self.date and self.date < Date.today():
#            raise UserError(_("You can't use a date before today."))
