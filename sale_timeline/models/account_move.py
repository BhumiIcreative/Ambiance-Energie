...
class AccountMove(models.Model):
    ...
    payment_timeline_ids = One2many('payment.timeline', 'move_id', string=_('Payment timeline'))
    is_timeline = Boolean(_('Is Timeline ?'), compute='_cpt_is_timeline', store=True, default=False)
    payment_instrument_id = Many2one('sale_timeline.payment.instrument', string=_('Payment method'))
    amount_missing_from_timeline = Monetary(string=_("Amount missing from payment timeline"), compute='_compute_amount_missing_from_timeline')
    advance = Monetary('Advance', digits=(14,10))
    advance_payment_ids = One2many('account.payment', 'ac_id', string="advance payments")
    ...
    def _process_timeline(self, validate_timeline=False):
        ...
        if not self._context.get('force_bypass_timeline', False):
            ...
            if self.invoice_payment_term_id.name == "Echéancier de paiement" and not self.payment_timeline_ids:
                amount = (self.advance if self.advance >0 else 0) + (self.edf_prime if self.edf_prime else 0)
    ...
    def action_post(self):
        ...
        if self.type in ['out_invoice', 'out_refund'] and self.type_invoice != 'gran':
            ####################
            # variables en dur #
            ####################
            ...
            if self.type == 'out_invoice':       # facture client
                ...
                amount = self.amount_total - (self.edf_prime if self.edf_prime else 0)
            elif self.type == 'out_refund':      # avoir client
                var_payment_type =  'outbound'  # décaissement
                amount = self.amount_total - (self.edf_prime if self.edf_prime else 0)
            if self.is_timeline:
                for payment_timeline_id in self.payment_timeline_ids.filtered(lambda x: x.advance != True):
                    if (not payment_timeline_id.payment_instrument_id):
                        raise UserError("Please add a payment instrument on all the payment timelines")
                    self.env['account.payment'].create({
                        ...
                        self.env['account.payment'].create({
                            ...
                            'journal_id': var_journal_id,
                            'payment_method_id': self.env.ref('account.account_payment_method_manual_in').id,
                            'communication': self.name + ' / ' + (self.payment_instrument_id.name if self.payment_instrument_id else ''),
                            'company_id': self.company_id.id,
                            'oci_point_of_sale': self.oci_point_of_sale.id,
