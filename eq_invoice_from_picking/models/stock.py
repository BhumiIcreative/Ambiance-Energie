...
class wizard_stock_picking_invoice(models.TransientModel):
    ...
    def create_invoice(self):
        ...
        if 'internal' in picking_code_lst or 'outgoing' in picking_code_lst:
            raise ValidationError(_("Select only Receipts for create Invoice."))
        ...
        for lst_items in groupby_lst.values():
            ...
            for picking in lst_items:
                ...
                invoice_vals = picking.with_context(journal_id=self.journal_id.id, invoice_date=self.invoice_date, invoice_type=self.invoice_type)._prepare_invoice()
